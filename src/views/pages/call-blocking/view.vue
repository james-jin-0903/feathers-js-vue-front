<template>
  <div class="page-table scrollable only-y">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="voxo-square-icon voxo-bg-red">
              <v-svg class="voxo-white" sprite="call-block"/>
            </div>
          </template>
        </el-table-column>
        <el-table-column
    ...
    <el-dialog id="edit-modal" title="View / Edit Blocked Caller" :visible.sync="editVisible" :fullscreen="true">

    </el-dialog>

    <el-dialog id="create-modal" title="Create Call Blocking" :visible.sync="createVisible" :fullscreen="true">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('call-blocking', {findBlacklist: 'find'}),
      filteredData() {
        return this.tableData.filter(data => !this.search
          || data.callerId.toString().toLowerCase().includes(this.search.toLowerCase())
          || data.reason.toLowerCase().includes(this.search.toLowerCase())
          || data.inserted.toLowerCase().includes(this.search.toLowerCase())
        )
      },
      ...
      populateTable() {
        ...
        this.findBlacklist(params).then((res) => this.tableData = res);
      },
      exportTable() {
        exportExcel('call-blocking');
      }
    },
    mounted() {
      if (this.user) {
