<template>
  <div class="page-table scrollable only-y">
    ...
    <div class="table-box page-header card-base card-shadow--medium">
      <h1>
        <v-svg class="voxo-aqua" sprite="group-3"/>
        Conference Bridges
      </h1>
      <div class="flex">
        <div class="box">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="voxo-square-icon voxo-bg-aqua">
              <v-svg class="voxo-white" sprite="group-3"/>
            </div>
          </template>
        </el-table-column>
        <el-table-column
    ...
    <el-dialog id="edit-modal" title="View / Edit Conference Bridge" :visible.sync="editVisible" :fullscreen="true">

    </el-dialog>

    <el-dialog id="create-modal" title="Create Conference Bridge" :visible.sync="createVisible" :fullscreen="true">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('conference-bridges', {findConferenceBridges: 'find'}),
      filteredData() {
      ...
      populateTable() {
        ...
        this.findConferenceBridges(params).then((res) => this.tableData = res);
      },
      exportTable() {
        exportExcel('conference-bridges');
      }
    },
    mounted() {
      if (this.user) {
