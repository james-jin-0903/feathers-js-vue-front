<template>
  <div class="page-table scrollable only-y">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="voxo-square-icon voxo-bg-green">
              <v-svg class="voxo-white" sprite="megaphone"/>
            </div>
          </template>
        </el-table-column>
        <el-table-column
    ...
    <el-dialog id="edit-modal" title="View / Edit Paging Group" :visible.sync="editVisible" :fullscreen="true">

    </el-dialog>

    <el-dialog id="create-modal" title="Create Paging Group" :visible.sync="createVisible" :fullscreen="true">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('paging-groups', {findPagingGroups: 'find'}),
      filteredData() {
      ...
      populateTable() {
        ...
        this.findPagingGroups(params).then((res) => this.tableData = res);
      },
      exportTable() {
        exportExcel('paging-groups');
      }
    },
    mounted() {
      if (this.user) {
