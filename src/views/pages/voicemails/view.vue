<template>
  <div class="page-table scrollable only-y">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="voxo-square-icon voxo-bg-purple">
              <v-svg class="voxo-white" sprite="voicemail-2"/>
            </div>
          </template>
        </el-table-column>
        <el-table-column
    ...
    <el-dialog id="edit-modal" title="View / Edit Voicemail" :visible.sync="editVisible" :fullscreen="true">

    </el-dialog>

    <el-dialog id="create-modal" title="Create Voicemail" :visible.sync="createVisible" :fullscreen="true">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions('voicemails', {findVoicemails: 'find'}),
      filteredData() {
        return this.tableData.filter(data => !this.search
          || data.mailbox.toString().toLowerCase().includes(this.search.toLowerCase())
          || data.password.toLowerCase().includes(this.search.toLowerCase())
          || !!data.fullName && data.fullName.toLowerCase().includes(this.search.toLowerCase())
      ...
      populateTable() {
        ...
        this.findVoicemails(params).then((res) => this.tableData = res);
      },
      exportTable() {
        exportExcel('voicemails');
      }
    },
    mounted() {
      if (this.user) {
